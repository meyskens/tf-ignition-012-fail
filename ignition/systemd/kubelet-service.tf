...
data "ignition_systemd_unit" "kubelet_controller_service" {
  ...
  content = element(
    data.template_file.kubelet_controller_service.*.rendered,
    count.index,
  )
}
...
data "template_file" "ml_kubelet_controller_service" {
  count    = length(var.controller_domains)
  template = file("${path.module}/files/ml-kubelet-controller.service.tpl")
...
data "ignition_systemd_unit" "ml_kubelet_controller_service" {
  count = length(var.controller_domains)
  name  = "kubelet.service"
  content = element(
    data.template_file.ml_kubelet_controller_service.*.rendered,
    count.index,
  )
}

output "ml_kubelet_controller_service" {
  value = data.ignition_systemd_unit.ml_kubelet_controller_service.*.id
}
...
data "ignition_systemd_unit" "kubelet_worker_service" {
  ...
  name  = "kubelet.service"
  content = element(
    data.template_file.kubelet_worker_service.*.rendered,
    count.index,
  )
}
...
data "ignition_systemd_unit" "kubelet_elasticsearch_worker_service" {
  ...
  name  = "kubelet.service"
  content = element(
    data.template_file.kubelet_elasticsearch_worker_service.*.rendered,
    count.index,
  )
}
...
data "ignition_systemd_unit" "kubelet_jenkins_worker_service" {
  ...
  name  = "kubelet.service"
  content = element(
    data.template_file.kubelet_jenkins_worker_service.*.rendered,
    count.index,
  )
}
...
data "ignition_systemd_unit" "kubelet_rabbitmq_worker_service" {
  ...
  name  = "kubelet.service"
  content = element(
    data.template_file.kubelet_rabbitmq_worker_service.*.rendered,
    count.index,
  )
}
...
data "ignition_systemd_unit" "kubelet_postgres_worker_service" {
  ...
  name  = "kubelet.service"
  content = element(
    data.template_file.kubelet_postgres_worker_service.*.rendered,
    count.index,
  )
}
...
data "ignition_systemd_unit" "kubelet_prometheus_worker_service" {
  ...
  name  = "kubelet.service"
  content = element(
    data.template_file.kubelet_prometheus_worker_service.*.rendered,
    count.index,
  )
}
